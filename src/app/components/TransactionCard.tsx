   ...
   <div className={`flex justify-between w-full items-center`}>
        <div className={`flex items-center gap-2`}>
            ...
            <div className={`flex flex-col gap-2`}>
                ...
                <p className={` text-sm font-medium ${type === 'withdrawal' && status === 'successful' ? 'text-[#0EA163]' : type === 'withdrawal' && status === 'pending' ? 'text-[#A77A07]' : 'text-[#56616B]'  } `}>{ type === 'withdrawal' ? (status ? capitalizeFirstLetter(status) : '') : (name ? name : '') } </p>
            </div>
        </div>
        <div className={`flex flex-col gap-2`}>
